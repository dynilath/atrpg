...
from ..deps import get_config as _get_web_config
...
def _find_config_toml() -> Path:
    """找到项目使用的 config.toml（与 web_api/config.py 一致）"""
    import os
...
# ═══════════════════════════════════════════════════════════════════
# AI 配置（读/写 bot/config.toml 的 [atrpg] 段）
# ═══════════════════════════════════════════════════════════════════
...
async def set_ai_config(body: dict[str, str]):
    ...
    try:
        ...
        def _set_key(section: str, key: str, value: str) -> str:
            ...
            # key 不存在：在对应 section 的末尾前插入
            # 找到 [atrpg] 段，在其最后一个 key=value 后插入
            # 简单做法：追加到 atrpg 段末尾
            lines = raw.split("\n")
            result_lines = []
            in_section = False
