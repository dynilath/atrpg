...
无 NoneBot 依赖，可被 web_api 和 bot 共享。
"""
...
def session_list_turns(root: Path) -> list[dict]:
    ...
    with sqlite3.connect(str(db)) as conn:
        conn.row_factory = sqlite3.Row
        rows = conn.execute(
            "SELECT t.id, t.turn_no, t.parent_id, t.meta, b.name as branch_name, b.id as branch_id "
            "FROM tree_nodes t "
            "LEFT JOIN branches b ON b.head_id = t.id OR t.id IN ("
            "  SELECT t2.id FROM tree_nodes t2 "
            "  INNER JOIN branches b2 ON b2.head_id = t2.id"
            ") "
            "ORDER BY t.turn_no"
        ).fetchall()
        result = []
...
def session_get_turn_detail(root: Path, turn_id: str) -> dict | None:
    ...
    with sqlite3.connect(str(db)) as conn:
        conn.row_factory = sqlite3.Row
        row = conn.execute(
            "SELECT id, turn_no, snapshot_path, meta FROM tree_nodes WHERE id = ?",
            (turn_id,),
        ).fetchone()
        if not row:
            return None
...
def session_create_branch(root: Path, from_node_id: str, name: str | None = None) -> str | None:
    ...
    with sqlite3.connect(str(db)) as conn:
        ...
        branch_id = name or f"branch-{node[1]}"
        # 确保不重名
        base = branch_id
        n = 1
        while conn.execute("SELECT 1 FROM branches WHERE id = ?", (branch_id,)).fetchone():
            branch_id = f"{base}-{n}"
            n += 1

        conn.execute(
            "INSERT INTO branches (id, name, head_id, created_at) VALUES (?,?,?,?)",
            (branch_id, branch_id, from_node_id, datetime.now(timezone.utc).isoformat()),
        )
        conn.execute("UPDATE active_branch SET branch_id = ? WHERE id = 1", (branch_id,))
        conn.commit()
        logger.info(f"session create branch: {branch_id} from node {from_node_id[:8]}")
        return branch_id
